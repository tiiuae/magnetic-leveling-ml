...
def average_model_state(pth_files, n):
    avg_state = OrderedDict()
    for path in pth_files[-n:]:
        state = torch.load(path, map_location='cpu')['model']
        for k, v in state.items():
            avg_state[k] = avg_state.get(k, 0) + v / n
    return {'model': avg_state}

def save_averaged_checkpoints(pth_files_sorted, save_dir):
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)
    saved_files = []

    for n in [3, 5, 7, 10]:
        if len(pth_files_sorted) >= n:
            averaged_ckpt = average_model_state(pth_files_sorted, n)
            out_path = save_dir / f"averaged_last_{n}.pth"
...
def main():
    ...
    avg_files = save_averaged_checkpoints(pth_files_sorted, FINAL_DIR)
    final_results = []
    print(avg_files)
    for index,model_path in enumerate(list(avg_files)):
